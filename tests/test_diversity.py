...
def test_record_to_text_uses_jsonpath_field() -> None:
    record = {"query": "find hotels", "extraction": {"intent": "travel"}}
    assert record_to_text(record, "$.query") == "find hotels"


def test_record_to_text_unmatched_jsonpath_falls_back_to_json() -> None:
